...
// --- NEW: Icon for professional error message ---
...
// --- NEW: Professional Notification Component ---
...
const InfoPage = () => {
    ...
    const handleVote = async (pollId, selectedOptionIds) => {
        try {
            const { data: updatedPoll } = await axios.put(`/api/info/vote/${pollId}`, { optionIds: selectedOptionIds }, getAuthConfig());
    ...
    const handleEditVote = async (pollId, selectedOptionIds) => {
        try {
            const { data: updatedPoll } = await axios.put(`/api/info/vote/edit/${pollId}`, { optionIds: selectedOptionIds }, getAuthConfig());
            updatePostState(updatedPoll);
    ...
    const handleLike = async (postId) => {
        try {
            const { data: updatedPost } = await axios.put(`/api/info/like/${postId}`, {}, getAuthConfig());
    ...
    return (
        <div>
            ...
            <style>{`
               ...
               .feed-panel { display: flex; flex-direction: column; background: var(--white); border: 1px solid var(--border-color); border-radius: 1rem; overflow: hidden; height: calc(100vh - 180px); }
               ...
               /* --- NEW: Professional Notification Style --- */
...
const PostItem = ({ post, userId, onVote, onLike, onEditVote, showNotification, isModalView = false }) => {
    ...
    // --- FIX 1: New function to handle single-select edit ---
        ...
        return (
            <div className="post-item message-item">
                ...
                <div className="post-footer">
                    ...
                    <button className={`like-btn ${hasLiked ? 'liked' : ''}`} onClick={() => onLike(post._id)}>
                        <ThumbsUpIcon filled={hasLiked} /> {post.likes.length}
                    </button>
                </div>
            </div>
        );
                ...
                <div className="poll-options">
                    {post.pollOptions.map(option => {
                        ...
                        if (userVotes.length > 0 && !isEditingVote) {
                            return (
                                <div key={option._id} className={`poll-option voted ${userVotes.includes(option._id) ? 'my-vote' : ''}`}>
                        ...
                        if (post.isMultiSelect) {
                            return (
                                <div key={option._id} className="multi-poll-option">
                                    <input type="checkbox" id={`${post._id}-${option._id}`} checked={selectedOptions.includes(option._id)} onChange={() => handleMultiSelectChange(option._id)} />
                                    <label htmlFor={`${post._id}-${option._id}`}>{option.text}</label>
                                </div>
                            );
                        } else {
                             return (
                                <div key={option._id} className="poll-option" onClick={() => (isEditingVote ? handleSingleSelectEdit(option._id) : onVote(post._id, [option._id]))}>
                                    <div className="poll-option-text"><span>{option.text}</span></div>
                                </div>
                            );
                        }
                    })}
                    {(post.isMultiSelect && (userVotes.length === 0 || isEditingVote)) && (
                        <button onClick={handleVoteSubmit} className="submit-vote-btn">{isEditingVote ? 'Update Vote' : 'Submit Vote'}</button>
                    )}
                </div>
                 <div className="post-footer">
                    <span className="timestamp">{new Date(post.createdAt).toLocaleString()}</span>
